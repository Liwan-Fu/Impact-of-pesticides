...
dat_list1=lapply(unique(combined$Taonomic_group_response),
                 function(x){subset(combined,Taxoomic_group_response==x)})
...
colnames(S4)<-c("#Obs","#Studies","Effect size","T-value","Df",
                "P-value","CL.lb","CL.ub")
write.xlsx(S4,"Supplementary Table 47.xls")
...
dat_list3=c(lapply(unique(dat_list2[[1]]$Taxonmic_group_response),
                   function(x) {
                     subset(dat_list2[[1]],Taxonomic_group_response==x)
                   }),
            lapply(unique(dat_list2[[2]]$Taxonomic_group_response),
                   function(x) {
                     subset(dat_list2[[2]],Taxonoic_group_response==x)
                   }),
            lapply(unique(dat_list2[[3]]$Taxonomic_group_response),
                   function(x) {
                     subset(dat_list2[[3]],Taxnomic_group_response==x)
                   }),
            ...
            lapply(unique(dat_list2[[5]]$Taxonomic_group_response),
                   function(x) {
                     subset(dat_list2[[5]],Taxnomic_group_response==x)
                   }),
...
colnames(S5)<-c("#Obs","#Studies","Effect size","T-value","Df",
                "P-value","CL.lb","CL.ub")
write.xlsx(S5,"Supplementary Table 48.xls")
...
S6_category=c(unique(combined$pesticide_by_source),
              unique(combined$Primary.classifiation.of.insecticide........一级分类))[c(1,3,2,4,7,10,6,8,5,9,11)]
...
for (i in 1:length(S6_category)) {
  for (j in 1:length(dat_list1)){
      ...
      mydat=subset(dat_list1[[j]],
                   Primary.classificatio.of.insecticide........一级分类==S6_category[i])
      if (nrow(mydat)<=1) {
        S6[(i-1)*length(dat_list1)+j,]=c(nrow(mydat),length(unique(mydat$Code)),
                                         rep(NA,6))
      } else {
...
colnames(S6)<-c("#Obs","#Studies","Effect size","T-value","Df",
                "P-value","CL.lb","CL.ub")
write.xlsx(S6,"Supplementary Table 49.xls")

########## Supplementary Table 50 ########### 
Pesticide_category=c('Pesticide',
                     unique(combined$pesticide_by_target_organisms))
dat_list1=lapply(unique(combined$Taxonic_group_response),
                 function(x){subset(comined,Taxonomic_group_response==x)})
...
colnames(S7)<-c("#Obs","#Studies","Effect size","T-value","Df",
                "P-value","CL.lb","CL.ub")
write.xlsx(S7,"Supplementary Table 50.xls")
...
S8=matrix(NA,length(Experiment)*length(Pesticide_ctegory)*length(dat_list1),8)
for (k in 1:length(Experiment)) {
  for (i in 1:length(Pesticide_category)) {
    for (j in 1:length(dat_list1)){
        mydat=subset(dat_list1[[j]],
...
colnames(S8)<-c("#Obs","#Studies","Effect size","T-value","Df",
                "P-value","CL.lb","CL.ub")
write.xlsx(S8,"Supplementary Table 51.xls")

########## Supplementary Table 52 ###########
Pesticide_category=c('Pesticide',
                     unique(combined$pesticide_by_target_organisms))
dat_list1=lapply(unique(combined$Taxomic_group_response),
                 function(x){subset(combined,Taxonomic_group_response==x)})
Experiment=unique(combined$Types.of.organism.eposure.to.pesticides)[c(2,1)]
...
colnames(S9)<-c("#Obs","#Studies","Effect size","T-value","Df",
                "P-value","CL.lb","CL.ub")
write.xlsx(S9,"Supplementary Table 52.xls")
...
dat_list10=c(lapply(unique(combined$Taxonomic_group),
             ...
             lapply(unique(combined$Taxonomic_group_response),
                    function(x){subset(combined,Taxonomic_group_response==x)}))
mycat=unique(combined$Taxnomic_group_response)
...
insecti=sort(unique(combined$pestcide_by_source))
S10=list()
for (k in 1:length(insecti)) {
  insecti_cate=sort(unique(subset(combined,
                                  pesticide_by_target_oganisms=='insecticides'&pesticide_by_source==insecti[k])$Secondary.classification.of.insecticide........二级分类))
  ...
  for (i in 1:length(insecti_cate)) {
    for (j in 1:length(dat_list10)){
      mydat=subset(dat_list10[[j]],
                   pestcide_by_target_organisms=='insecticides'&pesticide_by_source==insecti[k]&Seconday.classification.of.insecticide........二级分类==insecti_cate[i])
      if (nrow(mydat)<=1) {
...
colnames(S10)<-c('Insecticide','Insecticide category','Non-target organism category',
                 ...
                 "P-value","CL.lb","CL.ub")
write.xlsx(S10,"Supplementary Table 53.xls",row.names = F)
...
for (k in 1:length(fungici)) {
  fungici_cate=sort(unique(subset(combined,
                                  pesticide_by_target_orgaisms=='fungicides'&pesticide_by_souce==fungici[k])$Secondary.classification.of.insecticide........二级分类))
...
colnames(S11)<-c('fungicides','fungicides category','Non-target organism category',
                 "#Obs","#Studies","Effect size","T-value","Df",
                 "P-value","CL.lb","CL.ub")
write.xlsx(S11,"Supplementary Table 54.xls",row.names = F)
...
for (k in 1:length(herbici)) {
  herbici_cate=sort(unique(subset(combined,
                                  pesticide_by_target_organsms=='herbicides'&pesticide_by_source==herbici[k])$Secondary.clssification.of.insecticide........二级分类))
...
colnames(S12)<-c('herbicides','herbicides category','Non-target organism category',
                 "#Obs","#Studies","Effect size","T-value","Df",
                 "P-value","CL.lb","CL.ub")
write.xlsx(S12,"Supplementary Table 55.xls",row.names = F)
...
datlis_S13=list(combined[combined$Taxonomic_group=='animals',],
                ...
                combined[combined$Priary.classification.of.animal......一级分类=='Vertebrate',],
                combined[combined$Taxnomic_group=='plants',],
                combined[combined$Primay.classification.of.animal......一级分类=='Seed plant',],
                combined[combined$Primary.classification.of.animal......一级分类=='Spore-producing  plant',],
                combined[combined$Taxonmic_group=='microorganisms',],
...
for (i in 1:length(datlis_S13)) {
  Indicator=unique(datlis_S13[[i]]$Taxonomic_group_response)
  S13[[i]]=matrix(NA,length(Indicator),9)
  for (j in 1:length(Indicator)) {
    mydat=subset(datlis_S13[[i]],
                 Taxonomic_group_response==Indicator[j])
    res1=rma(yi, vi, data = mydat,
             control=list(stepadj=0.5,maxiter=300),
             verbose=T)
    res2=trimfill(res1,
                  control=list(stepadj=0.5,maxiter=200),verbose=T)
...
colnames(S13)<-c('Phylogenetic group','Indicator',"Missing studies",
                 ...
                 "P-value","CL.lb","CL.ub")
write.xlsx(S13,"Supplementary Table 56.xls",row.names = F)
...
linear_table=function(dat,Fig_Source,columname,roname){
  ...
  for(i in 1:length(mylabel)) {
    subdat=subset(dat,Taxonomic_group_response==grep(mylabel[i],unique(dat$Taxonmic_group_response),value = T))
...
write.xlsx(S14,'Supplementary Table 57.xls',row.names = F)
