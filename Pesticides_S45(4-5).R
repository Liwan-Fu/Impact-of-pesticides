...
Animal_data <- read.table(paste(DataPre_path,"AllAnimal.txt",sep = ""),
                         header = T)
...
Matched_Plant_InCor <- match(Animal_data$AnLn, rownames(Animal_Cor))
Animal_data <- Animal_data[!is.na(Matched_Plant_InCor),]

Animal_Cor <- Animal_Cor[order(dimnames(Animal_Cor)[[1]]), order(dimnames(Animal_Cor)[[1]])]
Animal_data$Plant_Species <- factor(Animal_data$AnLn)
levels(Animal_data$Plant_Species) <- sort(dimnames(Animal_Cor)[[1]])
...
Animal_data <- Animal_data[which(Animal_data$yi >= -2 & Animal_data$yi <=2),]
Animal_data <- Animal_data[match(Animal_data$McLn, animal$animal_species),]
...
colnames(Results) <- c("Predictor variables",
                       ...
                       "P",
                       "N")
write.csv(Results, "SuppTab45-2_1111.csv", row.names = F)
...
Micro_data <- read.table(paste(DataPre_path,"AllMicro.txt",sep = ""),
                        header = T)
...
Micro_data <- Micro_data[which(Micro_data$yi >= -2 & Micro_data$yi <=2),]
Micro_data <- Micro_data[match(Micro_data$McLn, micro$micro_species),]
...
colnames(Results) <- c("Predictor variables",
                       ...
                       "P",
                       "N")
write.csv(Results, "SuppTab45-5_1111_1104.csv", row.names = T)
