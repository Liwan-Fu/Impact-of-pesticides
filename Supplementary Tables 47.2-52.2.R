...
myphylo=function(Total.data2){
  ...
  A <- vcv(mytree, corr=TRUE)
  A2=round(A,1)
  levels(Total.data2$Plant.speces.new)=sort(dimnames(A)[[1]])
  Total.data2$Plant.speces.new.p=Total.data2$Plant.species.new
  return(list(Total.data2,A2))
}
# combined=na.omit(combined)
#  combined=combined[sample(nrow(combined))[1:500],]

########## Supplementary Table 47.2 ###########
S4=matrix(NA,12,8)
...
colnames(S4)<-c("#Obs","#Studies","Effect size","T-value","Df",
                "P-value","CL.lb","CL.ub")
write.xlsx(S4,"Supplementary Table 47.2.xls")

########## Supplementary Table 48.2 ###########
...
dat_list3=c(lapply(unique(dat_list2[[1]]$Taxonomic_group_response),
                   function(x) {
                     subset(dat_list2[[1]],Taxonomic_goup_response==x)
                   }),
            lapply(unique(dat_list2[[2]]$Taxnomic_group_response),
                   function(x) {
                     subset(dat_list2[[2]],Taxonmic_group_response==x)
                   }))
...
S5<-as.data.frame(S5,row.names<-Rnames)
colnames(S5)<-c("#Obs","#Studies","Effect size","T-value","Df",
                "P-value","CL.lb","CL.ub")
write.xlsx(S5,"Supplementary Table 48.2.xls")

########## Supplementary Table 49.2 ###########
dat_list1=lapply(unique(combined$Taxonomic_group_response),
                   function(x){subset(combined,Taxonomic_group_response==x)})
S6_category=c(unique(combined$pesticide_by_source),
              unique(combined$Primary.classfication.of.insecticide........一级分类))
...
colnames(S6)<-c("#Obs","#Studies","Effect size","T-value","Df",
                "P-value","CL.lb","CL.ub")
write.xlsx(S6,"Supplementary Table 49.2.xls")

########## Supplementary Table 50.2 ###########
Pesticide_category=c('Pesticide',
                     unique(combined$pesticide_by_target_organisms))
dat_list1=lapply(unique(combined$Taxonoic_group_response),
                   function(x){subset(combined,Taxnomic_group_response==x)})
...
colnames(S7)<-c("#Obs","#Studies","Effect size","T-value","Df",
                "P-value","CL.lb","CL.ub")
write.xlsx(S7,"Supplementary Table 50.2.xls")

########## Supplementary Table 51.2 ###########
Pesticide_category=c('Pesticide',
                     unique(combined$pesticide_by_target_organisms))
dat_list1=lapply(unique(combined$Taxonomic_group_response),
                   function(x){subset(combined,Taxonomic_group_response==x)})
...
for (k in 1:length(Experiment)) {
  for (i in 1:length(Pesticide_category)) {
    for (j in 1:length(dat_list1)){
        mydat=subset(dat_list1[[j]],
                     Major.climatic.zones==Experiment[k]&pestiide_by_target_organisms==Pesticide_category[i]&Experiment.type=='Field')
      if (nrow(mydat)<=1) {
...
colnames(S8)<-c("#Obs","#Studies","Effect size","T-value","Df",
                "P-value","CL.lb","CL.ub")
write.xlsx(S8,"Supplementary Table 51.2.xls")

########## Supplementary Table 52.2 ###########
Pesticide_category=c('Pesticide',
                     unique(combined$pesticide_by_target_organisms))
dat_list1=lapply(unique(combined$Taxonomic_group_response),
                   function(x){subset(combined,Taxonomic_group_response==x)})
...
for (k in 1:length(Experiment)) {
  for (i in 1:length(Pesticide_category)) {
    for (j in 1:length(dat_list1)){
        mydat=subset(dat_list1[[j]],
                     Types.of.organism.exposure.to.pestiides==Experiment[k]&pesticide_by_target_organisms==Pesticide_category[i])
      if (nrow(mydat)<=1) {
...
colnames(S9)<-c("#Obs","#Studies","Effect size","T-value","Df",
                "P-value","CL.lb","CL.ub")
write.xlsx(S9,"Supplementary Table 52.2.xls")
