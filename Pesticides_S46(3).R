...
mycor=V.PhyloMaker::phylo.maker(data.frame(species=a3$species,genus=a3$genus,
                                           family=a3$family))
...
system.time(model1<-rma.mv(yi,vi,mods=~factor(Taxonomic_group)-1+vi,
...
S3_model1=matrix(NA,17,2)
for (i in 1:length(myindex)) {
  if(i<length(myindex)) {
        modelB<-rma.mv(yi,vi,mods=~factor(Taxonomic_group)+factor(mydat[[1]][,myindex[i]])+factor(Taxonomic_group):factor(mydat[[1]][,myindex[i]])-1+vi,
                       random =list( ~ 1 | factor(Insecticide.name),
                                     ~ 1 | Code,
                                     ~ 1 | Plant.species.new,
                                     ~ 1|Plant.species.new.p),
                       R=list(Plant.species.new.p=mydat[[2]]),
                       data=mydat[[1]])
        modelA<-rma.mv(yi,vi,mods=~factor(Taxonomic_group)+factor(mydat[[1]][,myindex[i]])-1+vi,
                       random =list( ~ 1 | factor(Insecticide.name),
                                     ~ 1 | Code,
                                     ~ 1 | Plant.species.new,
                                     ~ 1|Plant.species.new.p),
    ...
    modelB<-rma.mv(yi,vi,mods=~factor(Taxonomic_group)+xi+factor(Taxonomic_group):xi-1+vi,
                   random =list( ~ 1 | factor(Insecticide.name),
                                 ~ 1 | Code,
                                 ~ 1 | Plant.species.new,
                                 ~ 1|Plant.species.new.p),
                   R=list(Plant.species.new.p=mydat[[2]]),
                   data=mydat[[1]])
    modelA<-rma.mv(yi,vi,mods=~factor(Taxonomic_group)+xi-1+vi,
                   random =list( ~ 1 | factor(Insecticide.name),
                                 ~ 1 | Code,
                                 ~ 1 | Plant.species.new,
                                 ~ 1|Plant.species.new.p),
  ...
  S3_model1[1,]=c(tail(model1[["zval"]],1),tail(model1[["pval"]],1))
  S3_model1[2*i,]=c(tail(modelA[["zval"]],1),tail(modelA[["pval"]],1))
  S3_model1[2*i+1,]=c(tail(modelB[["zval"]],1),tail(modelB[["pval"]],1))
}


model2<-rma.mv(yi,vi,mods=~factor(Taxonomic_group_response)-1+vi,
               random =list( ~ 1 | factor(Insecticide.name),
                             ~ 1 | Code,
                             ~ 1 | Plant.species.new,
                             ~ 1|Plant.species.new.p),
               R=list(Plant.species.new.p=mydat[[2]]),
               data=mydat[[1]])
S3_model2=matrix(NA,17,2)
S3_model2[1,]=c(tail(model2[["zval"]],1),tail(model2[["pval"]],1))
for (i in 1:length(myindex)) {
  if(i<length(myindex)) {
        modelB<-rma.mv(yi,vi,mods=~factor(Taxonomic_group_response)+factor(mydat[[1]][,myindex[i]])+factor(Taxonomic_group_response):factor(mydat[[1]][,myindex[i]])-1+vi,
                       random =list( ~ 1 | factor(Insecticide.name),
                                     ~ 1 | Code,
                                     ~ 1 | Plant.species.new,
                                     ~ 1|Plant.species.new.p),
                       R=list(Plant.species.new.p=mydat[[2]]),
                       data=mydat[[1]])
        modelA<-rma.mv(yi,vi,mods=~factor(Taxonomic_group_response)+factor(mydat[[1]][,myindex[i]])-1+vi,
                       random =list( ~ 1 | factor(Insecticide.name),
                                     ~ 1 | Code,
                                     ~ 1 | Plant.species.new,
                                     ~ 1|Plant.species.new.p),
    ...
    modelB<-rma.mv(yi,vi,mods=~factor(Taxonomic_group_response)+xi+factor(Taxonomic_group_response):xi-1+vi,
                   random =list( ~ 1 | factor(Insecticide.name),
                                 ~ 1 | Code,
                                 ~ 1 | Plant.species.new,
                                 ~ 1|Plant.species.new.p),
                   R=list(Plant.species.new.p=mydat[[2]]),
                   data=mydat[[1]])
    modelA<-rma.mv(yi,vi,mods=~factor(Taxonomic_group_response)+xi-1+vi,
                   random =list( ~ 1 | factor(Insecticide.name),
                                 ~ 1 | Code,
                                 ~ 1 | Plant.species.new,
                                 ~ 1|Plant.species.new.p),
  ...
  S3_model2[2*i,]=c(tail(modelA[["zval"]],1),tail(modelA[["pval"]],1))
  S3_model2[2*i+1,]=c(tail(modelB[["zval"]],1),tail(modelB[["pval"]],1))
}
...
S3=cbind(Predictor,Ref,rbind(S3_model1,S3_model2),n)
colnames(S3)=c('Predictor','Ref','Regression test value','P','n')
write.xlsx(S3,'Supplementary table 46(3).xls',row.names = F)
save.image(file = 'Supplementary Table 46(3).Rdata')
