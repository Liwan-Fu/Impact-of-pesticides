...
for (i in 1:length(myindex)) {
  if(i<length(myindex)) {
        modelB<-rma.mv(yi,vi,mods=~factor(Taxonomic_group)+factor(combined[,myindex[i]])+factor(Taxonomic_group):factor(combined[,myindex[i]])-1+vi,
                       random = list(~ 1 | factor(Insecticide.name),~1|Code),
                       data=combined)
        modelA<-rma.mv(yi,vi,mods=~factor(Taxonomic_group)+factor(combined[,myindex[i]])-1+vi,
                       random = list(~ 1 | factor(Insecticide.name),~1|Code),
                       data=combined)
    }
  }
...
for (i in 1:length(myindex)) {
  if(i<length(myindex)) {
        modelB<-rma.mv(yi,vi,mods=~factor(Taxonomic_group_response)+factor(combined[,myindex[i]])+factor(Taxonomic_group_response):factor(combined[,myindex[i]])-1+vi,
                       random = list(~ 1 | factor(Insecticide.name),~1|Code),
                       data=combined)
        modelA<-rma.mv(yi,vi,mods=~factor(Taxonomic_group_response)+factor(combined[,myindex[i]])-1+vi,
                       random = list(~ 1 | factor(Insecticide.name),~1|Code),
                       data=combined)
    }
  }
...
write.xlsx(S3,'Supplementary table 46(1).xls',row.names = F)
save.image(file = 'Supplementary Table 46(1).Rdata')
